...
const AllNotes = () => {
    ...
    const getNotesData = async () => {
        try {
            const notesResponse = await axios.get('http://localhost:3000/api/notes/all/');
            setNotes(notesResponse.data.allNotes);
    ...
    const filteredNotes = notes.filter(note =>
        ...
        note.categories.some(category => category.toLowerCase().includes(searchQuery.toLowerCase()))
    );
        ...
        <>
            {
                ...
                (
                <>
                    <h1 className='text-xl font-semibold mx-auto w-[70%] mb-1' >All the notes are from public directory </h1 >
                    <div className="mx-auto w-[70%] border-2 rounded-md py-1 border-fuchsia-200 px-2 flex">
                        <input
                            type="text"
                        ...
                        />
                        <box-icon name='search-alt' color='#cacaca'></box-icon>
                    </div>
                    {/* {console.log(notes)} */}
                    {/* {console.log(users)} */}
                    {
                        users.map((user, userIndex) => (
                            <div key={userIndex} className='w-[70%] mx-auto'>
                                {filteredNotes.filter(note => note.user === user._id).map((note, noteIndex) => (
                                    <div key={noteIndex} className={`my-5 p-0  ${activeNoteId ? ('border-2 border-indigo-200 rounded-md') : ('')}`}>
                                        <div onClick={() => toggleAccordion(note._id)} className={`cursor-pointer ${activeNoteId ? ('p-2') : ('border-2 border-indigo-200 rounded-md p-2 ')}`}>
                                            <div className="flex justify-between gap-3">
                                                <h1 className='font-semibold'>{note.title}</h1>
                                                <div className="text-gray-400">Author: {user.name}</div>
                                            </div>
                                        </div>
                                        <div
                                            id={`hs-collapse-${note._id}`}
                                            aria-labelledby={`hs-heading-${note._id}`}
                                            className={`mb-2 px-2 py-1 hs-accordion-content overflow-hidden transition-[height] duration-300  ${activeNoteId === note._id ? 'block' : 'hidden'}`}
                                        >
                                            ...
                                            {note.pdfs && note.pdfs.length > 0 && (
                                                <div className="mt-4">
                                                    ...
                                                    <ul>
                                                        {note.pdfs.map((pdfId) => (
                                                            <li key={pdfId}>
                                                                {pdfId !== 'undefined' && pdfPaths[pdfId] ? (
                                                                    <span className="px-3 py-1 bg-green-300 rounded-md">
                                                                        <a
